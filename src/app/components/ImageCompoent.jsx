...
const ImageFollowCursor = () => {
  ...
  const [position, setPosition] = useState({ x: isBrowser ? window.innerWidth / 2 : 0, y: isBrowser ? window.innerHeight / 2 : 0 });
  const [targetPosition, setTargetPosition] = useState({ x: isBrowser ? window.innerWidth / 2 : 0, y: isBrowser ? window.innerHeight / 2 : 0 });
